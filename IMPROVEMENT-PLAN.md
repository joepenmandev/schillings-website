...
| **P1.2** | Shorten redirect chains | No **high-traffic** URL with **>1** 301 hop where avoidable. **Current IA:** **`/expertise/…`** is canonical; **`/services/…`** is not published on the Astro site, and **no** **`/expertise` → `/services`** redirects were added while the new stack is pre-production. Populate **`redirect-map.csv`** / CDN rules at cutover (legacy host → **`/expertise/…`**) and re-verify **Internal → Redirect chains** in Screaming Frog after deploy. | [Redirect chains](https://www.screamingfrog.co.uk/seo-spider/issues/response-codes/internal-redirect-chains/) |
